...
1. **Почта.** Во всех страницах стоит заглушка `support@allikastudio.eu`
   (футер + юридические страницы сайта). Заменить на настоящий адрес:
   ```
   grep -rl "support@allikastudio.eu" . | xargs sed -i '' 's/support@allikastudio.eu/НОВАЯ@ПОЧТА/g'
   ```
...
| | про что | адрес |
|---|---|---|
| `legal/privacy-policy/`, `legal/terms-of-use/` | сам сайт allikastudio.eu | — |
...
`https://allikastudio.eu/benzina/privacy-policy/`.
...
## Как выложить: плоская сборка

Веб-загрузчик GitHub принимает папки только перетаскиванием, а оно работает не
во всех браузерах. Поэтому есть плоская сборка: `python3 tools/build-flat-upload.py`
собирает папку `upload/` — все 71 файл в одном уровне, ссылки между страницами
переписаны на новые имена (`projects/infur/index.html` -> `projects-infur.html`).

Загружать нужно **содержимое** `upload/`, а не саму папку. Сама `upload/` — сборка,
её не редактируют: правки вносятся в обычные файлы сайта, потом сборка запускается
заново.

## Как выложить на GitHub Pages (с папками)
...
5. Свой домен (allikastudio.eu) подключается там же, в Settings → Pages →
